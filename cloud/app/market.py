...
def closed_hourly_bars(symbol, limit=200):
    ...
    if len(df)<required: raise RuntimeError(f"insufficient completed bars for {symbol}: {len(df)}")
    # integrity: exact 1h continuity across the requested calculation window
    d=df.open_time.tail(required).diff().dropna().dt.total_seconds()
    if not (d==3600).all(): raise RuntimeError(f"1h candle gap detected for {symbol}")
    return df
